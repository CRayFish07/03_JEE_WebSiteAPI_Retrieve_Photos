...
import com.joe.web.SaveImageThread;
import com.opensymphony.xwork2.ActionContext;
import com.opensymphony.xwork2.ActionSupport;

public class SearchPhotosAction extends ActionSupport {
	//keyword of user input from screen
	private String keyword = null;
	//photo urls from the flickr returns response.
...
//			returnedList.add(buffer.toString());
		...
		//this block is commented, because struts2 and ajax communicated in json format already
		/*
		Gson gson = new Gson();
		//String jsonResult = gson.toJson(returnedList);
		String jsonResult = gson.toJson(urls);
...
//		HttpServletRequest request = (HttpServletRequest)ctx.get(ServletActionContext.HTTP_REQUEST);        
		...
		*/
		ActionContext ctx = ActionContext.getContext();
		HttpServletRequest request = (HttpServletRequest)ctx.get(ServletActionContext.HTTP_REQUEST);    
		request.getSession().setAttribute("UrlsOfPhotos", urls);
		request.getSession().setAttribute("KeywordOfSearch", keyword);
		return "SearchFlickr";
	}

	public String savePhotos() throws Exception {
		if (urls !=null) {
		...
		ActionContext ctx = ActionContext.getContext();
		HttpServletRequest request = (HttpServletRequest)ctx.get(ServletActionContext.HTTP_REQUEST);
		//request.getSession().getAttribute("UrlsOfPhotos", urls);
		keyword = (String)request.getSession().getAttribute("KeywordOfSearch");
		urls = (List<String>)request.getSession().getAttribute("UrlsOfPhotos");
		if (urls !=null) {
			System.out.println("urls:" + urls.size());
		} else {
			System.out.println("no photos to save.");
		...
		String path = request.getSession().getServletContext().getRealPath("/storeImage");
	...
	public String savePhotosInMultiThread() throws Exception {
		...
		//we suppose each thread will have 4 photos to save
		String[] array = null;
